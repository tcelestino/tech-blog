...
Se testes automatizados são ou não importantes não é o foco desse _post_. Vários outros já argumentaram em cima desse tema e, como nós acreditamos que seja importante, estamos de olho no código para melhorá-lo e garantir que tudo funcione corretamente. Para isso, usamos os _frameworks_ e dentre os diversos disponíveis, o AssertJ acabou de entrar na nossa _stack_. Novamente, não optamos por utilizá-lo, pois escrevemos menos. Ao contrário, escrevemos mais, mas fazemos nossos testes como se fossem frases que podem ser lidas, facilitando muito nosso trabalho. Aqui, vimos apenas uma parte dos recursos disponíveis, mas olhando no site oficial, encontraremos diversos outros que podem ser muito úteis, como o método `extracting()`, que permite retirarmos atributos do objeto em questão, ou criar _custom assertions_, ou seja, podemos ter nossas próprias asserções!